...
def next_tick_delay(anchor: float, n: int, interval: float) -> tuple[float, float, int]:
    """計算下一個 tick 的 sleep delay（絕對時間錨定）。

    計算方式：
        next_tick = anchor + (n + 1) * interval
        delay = next_tick - now

    三種情況：
        1. delay > 0（正常）：回傳 ``(delay, anchor, n)``，呼叫端 sleep(delay)。
        2. -interval < delay <= 0（輕微落後）：回傳 ``(0.0, anchor, n)``，呼叫端 sleep(0) 讓出 event loop。
        3. delay <= -interval（嚴重落後）：重設 anchor 為 now，n 歸零。
           回傳 ``(0.0, now, 0)``，呼叫端 sleep(0) 避免 burst catch-up。

    Args:
        anchor: Loop 啟動時的 monotonic 時間基準。
        n: 目前已完成的 tick 次數（從 0 開始計算，呼叫端負責在呼叫後遞增）。
        interval: 週期秒數。

    Returns:
        ``(delay, new_anchor, new_n)``：
        - ``delay``：應該 ``await asyncio.sleep(delay)`` 的秒數（含 0）。
        - ``new_anchor``、``new_n``：若嚴重落後則重設，否則不變。
