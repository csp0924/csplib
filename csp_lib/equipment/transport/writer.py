...
class ValidatedWriter:
    ...
    async def _write_to_device(self, point: WritePoint, encoded: list[int] | int | bool):
        ...
        if function_code == FunctionCode.WRITE_SINGLE_COIL:
            await self._client.write_single_coil(address=address, value=bool(encoded))
        elif function_code == FunctionCode.WRITE_SINGLE_REGISTER:
            await self._client.write_register(address=address, value=int(encoded))
        elif function_code == FunctionCode.WRITE_MULTIPLE_COILS:
            if isinstance(encoded, list):
                await self._client.write_coils(address=address, values=[bool(v) for v in encoded])
            else:
                await self._client.write_coils(address=address, values=[bool(encoded)])
        elif function_code == FunctionCode.WRITE_MULTIPLE_REGISTERS:
            if isinstance(encoded, list):
                await self._client.write_registers(address=address, values=encoded)
            else:
                await self._client.write_registers(address=address, values=[int(encoded)])
        else:
    ...
    async def _read_back(self, point: WritePoint) -> Any:
        ...
        if function_code in [FunctionCode.WRITE_SINGLE_COIL, FunctionCode.WRITE_MULTIPLE_COILS]:
            data = await self._client.read_coils(address, register_count)
        elif function_code in [FunctionCode.WRITE_SINGLE_REGISTER, FunctionCode.WRITE_MULTIPLE_REGISTERS]:
            data = await self._client.read_holding_registers(address, register_count)
        else:
            raise ValueError(f"不支援的 Function Code: {function_code}")
