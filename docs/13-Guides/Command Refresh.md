...
部分 PCS 韌體內建 5–10 s 的「setpoint watchdog」：若在此時間內未收到新 setpoint，PCS 自動歸零輸出。策略週期若超過此窗口，PCS 就會不斷閃斷。
...
> [!note] v0.9.0 Reconciler Protocol
> v0.9.0 起，`CommandRefreshService` 實作 `Reconciler` Protocol（相容保留舊 API）。新增 keyword-only 參數 `name="command_refresh"` 與 `status: ReconcilerStatus` 屬性、`reconcile_once()` 方法。詳見 [[Operator Pattern]]。
...
class CommandRefreshService(AsyncLifecycleMixin):
    def __init__(
        ...
        *,
        ...
        name: str = "command_refresh",    # v0.9.0 新增
    ) -> None: ...
```
