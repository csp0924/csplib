...
> [!note] 上層聚合（v0.9.2 已完成）
> `SystemController.describe()` 在 auto-discovery 時以 `ManagerDescribable.describe()` 取得 `StrategyExecutor` 的完整觀測快照，成為 `subsystems["executor"]`。詳見 [[SystemController#觀測介面：describe() 與 health()]]。
